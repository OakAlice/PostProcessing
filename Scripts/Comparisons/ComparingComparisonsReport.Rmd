...
Firstly I test whether there were interaction effects by doing an anova comparing the predictive performance of the model with and without the interactions.

```{r some stats}
# add the 2 information sources together
full_df <- merge(df, stat_data, by = "Species")

model_minimal <- lmer(relative_change ~ smoothing_type + Transition_Rate + Prop_Transitions + 
                   (1|Species), data = full_df, REML = FALSE)

model_med <- lmer(relative_change ~ smoothing_type + Transition_Rate + Prop_Transitions + 
                   smoothing_type:Transition_Rate +
                   smoothing_type:Prop_Transitions +
                   Transition_Rate:Prop_Transitions +
                   (1|Species), data = full_df, REML = FALSE)

model_full <- lmer(relative_change ~ smoothing_type * Transition_Rate * Prop_Transitions + 
                   (1|Species), data = full_df, REML = FALSE)
